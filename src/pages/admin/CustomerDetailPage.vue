...
const customer = computed(() => customers.find((c) => c.id === Number(route.params.id)))
...
<template>
  <div v-if="customer">
    <router-link to="/admin/pelanggan" class="inline-flex items-center gap-2 text-[0.8rem] text-muted transition hover:text-forest">
