...
const related = computed(() => (product.value ? relatedProducts(product.value) : []))
const qty = ref(1)
const tab = ref('deskripsi')
watch(() => route.params.slug, () => { qty.value = 1; tab.value = 'deskripsi' })

const soldOut = computed(() => product.value?.stock === 0)
const lowStock = computed(() => product.value && product.value.stock > 0 && product.value.stock <= 10)
...
const buyNow = () => {
  cart.add(product.value, qty.value, { silent: true })
  ...
  <div v-if="product">
    <div class="shell pt-8">
      <nav class="flex flex-wrap items-center gap-2 text-[0.72rem] uppercase tracking-[0.14em] text-muted" aria-label="Breadcrumb">
        <router-link to="/" class="transition hover:text-forest">Home</router-link>
        <span class="text-gold">/</span>
        <router-link :to="`/koleksi/${product.categorySlug}`" class="transition hover:text-forest">{{ product.category }}</router-link>
        <span class="text-gold">/</span>
