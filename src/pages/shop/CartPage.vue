      ...
      <section>
        <ul class="divide-y divide-line border-y border-line">
          <li v-for="item in cart.items.value" :key="item.id" class="flex gap-5 py-6">
            <router-link :to="`/produk/${item.slug}`" class="arch h-32 w-24 flex-none overflow-hidden border border-line sm:h-36 sm:w-28">
              ...
              <div class="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h2 class="mt-1.5 font-display text-[1.35rem] leading-tight">
                    <router-link :to="`/produk/${item.slug}`" class="link-underline">{{ item.name }}</router-link>
                  </h2>
        ...
        <router-link to="/koleksi" class="link-underline mt-8 inline-block text-[0.85rem] text-forest">Lanjut belanja</router-link>
      </section>
