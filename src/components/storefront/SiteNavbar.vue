...
<template>
  ...
  >
    <nav class="shell flex h-16 items-center justify-between gap-6 md:h-[72px]" aria-label="Utama">
      ...
      <router-link to="/" class="md:mr-8" aria-label="ArafahGift.id — beranda">
      ...
      <ul class="hidden flex-1 items-center gap-8 md:flex">
        <li v-for="link in links" :key="link.to">
          <router-link
            ...
            :class="route.path === link.to ? 'text-ivory' : ''"
          >
            {{ link.label }}
            <span v-if="route.path === link.to" class="absolute -bottom-1 left-0 right-0 h-px bg-gold" />
          </router-link>
        </li>
      </ul>
      ...
      <div class="flex items-center gap-1">
        ...
        <router-link to="/akun" class="hidden h-10 w-10 place-items-center text-ivory/75 transition hover:text-ivory sm:grid" aria-label="Akun saya">
        ...
        <div v-if="menuOpen" class="fixed inset-y-0 left-0 z-[120] flex w-[86%] max-w-sm flex-col bg-forest-deep md:hidden">
          ...
          <ul class="flex-1 overflow-y-auto px-5 pt-2">
            <li v-for="link in links" :key="link.to" class="border-b border-forest-soft/20">
              ...
              >
                {{ link.label }}
                <span v-if="route.path === link.to" class="h-1.5 w-1.5 rotate-45 bg-gold" />
              </router-link>
            </li>
          </ul>
          <!-- Drawer footer -->
          <div class="border-t border-forest-soft/20 px-5 py-6 space-y-4">
            <router-link to="/akun" class="flex items-center gap-2.5 text-[0.9rem] text-ivory/75 transition hover:text-ivory">
          ...
          <div class="shell py-5">
            ...
            <ul v-if="results.length" class="mt-4 space-y-1">
              <li v-for="p in results" :key="p.id">
                <router-link :to="`/produk/${p.slug}`" class="flex items-center gap-4 px-2 py-2.5 transition hover:bg-ivory">
