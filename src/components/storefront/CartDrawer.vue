        ...
        <div v-if="cart.items.value.length" class="flex-1 overflow-y-auto px-6">
          <ul class="divide-y divide-line">
            <li v-for="item in cart.items.value" :key="item.id" class="flex gap-4 py-5">
              <router-link :to="`/produk/${item.slug}`" class="arch h-24 w-[72px] flex-none overflow-hidden border border-line" @click="cart.closeDrawer()">
                ...
                <div class="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <h3 class="mt-1 font-display text-[1.1rem] leading-tight">
                      <router-link :to="`/produk/${item.slug}`" @click="cart.closeDrawer()">{{ item.name }}</router-link>
                    </h3>
                  </div>
