...
<template>
  ...
  >
    <div class="flex h-16 items-center justify-between border-b border-ivory/10 px-5">
      <router-link to="/"><BrandLogo tone="ivory" size="sm" /></router-link>
        ...
        <ul class="mt-3 space-y-0.5">
          <li v-for="item in g.items" :key="item.to">
            <router-link
              ...
              :class="$route.path === item.to || (!item.exact && $route.path.startsWith(item.to) && item.to !== '/admin')
    ...
    <div class="border-t border-ivory/10 px-5 py-4">
      <router-link to="/" class="text-[0.75rem] text-ivory/50 transition hover:text-gold">← Lihat storefront</router-link>
    </div>
  </aside>
</template>
