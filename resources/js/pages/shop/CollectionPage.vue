...
const activeCategory = computed(() => props.category ?? 'semua')
const activeOccasion = ref(props.untuk ?? null)
...
const categoryMeta = computed(() => props.categories.find((c) => c.slug === activeCategory.value))

const list = computed(() => {
  let out = props.products.filter((p) => p.status === 'active')
    ...
    <header class="border-b border-forest-soft/30 bg-forest-deep">
      <div class="shell py-14 sm:py-20">
        <nav class="flex items-center gap-2 text-[0.72rem] uppercase tracking-[0.14em] text-ivory/45" aria-label="Breadcrumb">
          <Link href="/" class="transition hover:text-ivory">Home</Link>
          <span class="text-gold">/</span>
          <Link href="/koleksi" class="transition hover:text-ivory">Koleksi</Link>
        ...
        <div class="space-y-8">
          <div>
            ...
            <ul class="mt-4 space-y-2.5">
              <li>
                <Link href="/koleksi" class="text-[0.85rem] transition" :class="activeCategory === 'semua' ? 'text-forest' : 'text-muted hover:text-forest'">
                  Semua produk
                </Link>
              </li>
